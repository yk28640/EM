...
Leader_P_ID int  Foreign key References Person_P(AutoID) ,
Assistant_P_ID int  Foreign key References Person_P(AutoID) ,
Direct_Member_P_ID int  Foreign key References Person_P(AutoID)
)
alter table DepartmentPerson_E4_P
Add constraint chk_E4_P check( 
                               (Leader_P_ID is null and Assistant_P_ID is null and Direct_Member_P_ID is not null) 
							    or
							   (Leader_P_ID is  not null and Assistant_P_ID is null and Direct_Member_P_ID is null) 
							    or
							    (Leader_P_ID is  null and Assistant_P_ID is not null and Direct_Member_P_ID is null) 
...
--P_ID int  Foreign key References Person_P(AutoID) not null ,
Group_E5_ID Int Foreign Key References Group_E5(AutoID) not null,
Leader_P_ID int  Foreign key References Person_P(AutoID) on delete cascade ,
Direct_Member_P_ID int  Foreign key References Person_P(AutoID) 
)
alter table GroupPerson_E5_P
add constraint GroupPerson_E5_P_Check check (
(Leader_P_ID is null and Direct_Member_P_ID is not null )or (Leader_P_ID is not null and Direct_Member_P_ID is  null))
...
add constraint Team_P_Check check(
(Leader_P_ID is null and Direct_Member_P_ID is not null ) or (Leader_P_ID is not null and Direct_Member_P_ID is null)
)
